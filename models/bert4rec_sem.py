...
class BERT4Rec(nn.Module):
    def __init__(self, hidden_units, max_seq_length, num_heads, num_layers, dropout_rate, pretrained_item_embeddings=None):
        """
        BERT4Rec with dynamic pretrained embeddings for zero-shot inference.
        Args:
        ...
        # Use pretrained embeddings or placeholders
        ...
        # Transformer Encoder layers (BERT-based architecture)
        self.attention_layers = nn.ModuleList([
            nn.TransformerEncoderLayer(
                ...
                activation="gelu"  # GELU activation for BERT-like models
    ...
    def forward(self, item_seq):
        ...
        # Attention mask to prevent attending to padding tokens
        attention_mask = (item_seq != 0)

        # Pass through Transformer layers
        for attn_layer in self.attention_layers:
            seq_emb = attn_layer(
                seq_emb.permute(1, 0, 2),  # (seq_len, batch_size, hidden_units)
                src_key_padding_mask=~attention_mask
            ).permute(1, 0, 2)

        # Apply layer normalization
        user_rep = self.layer_norm(seq_emb[:, -1, :])  # Use the last token as the user representation

        # Compute logits with dot product between user embedding and item embeddings
